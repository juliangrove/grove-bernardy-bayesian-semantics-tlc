...
main = do
  ...
  let x = evalFL $ normalForm $ expectedValue $ App (l1 (S (S Z))) (nf_to_λ $ u'' [Nothing]) ⋆ Lam (η (App (hmorph (S (S Z)) (App (Con $ General EqGen) (Pair (sel' 0 (upd' jp (upd' vlad emp))) jp))) (Var Get)))
  print (E.eval (\case) x :: Double)
